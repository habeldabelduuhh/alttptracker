(function (window) {
    ...
    window.receiveMessage = function (event) {
        if (window.origin === event.origin) {
            if (event.data.logic && flags.overworldshuffle != 'N') {
                ...
                for (var k = 0; k < dungeons.length; k++) {
                    var dungeonEntrances = new Array(dungeonEntranceCounts[k]),
                        dungeonEntrancesBunny = new Array(dungeonEntranceCounts[k]);
                    ...
                    if (flags.entrancemode != 'N') {//Not fully implemented (OW tracker doesn't know about dungeon locations), disable entrance chest colors for now
            ...
            else if (event.data == "OPENSETTINGS") {
    ...
    window.pushLocationObject = function (locations, entrance) {
        locations.push({
            'index': entranceNameToIndex[entrance.known_location],
            'friendly': getFriendlyName(entrance.known_location),
            'group': entranceNameToGroup[entrance.known_location],
            'location': entrance.caption,
            'note': entrance.note
        });
    }
    ...
    window.initializeSettings = function () {
        ...
        if (flags.mapmode != 'N') {
            ...
            if (flags.mapmode === 'C' || flags.mapmode === 'V') {
                var modal = document.getElementById("entranceModal"),
                    modalMain = document.getElementById("entranceModalMain");
