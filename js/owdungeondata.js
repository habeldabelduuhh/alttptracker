(function (window) {
    ...
    window.getFixedEdgePairs = function () {
        ...
        if (layoutshuffle === 'P') {
            ...
            if (similarow) {
                ...
                fixedEdges.push(getEdgesByKeys(0x30, "E0", 0x3A, "W0"));
                fixedEdges.push(getEdgesByKeys(0x30, "E2", 0x3A, "W2"));
            } else if (crossedow !== 'C' && (!mixedow || crossedow === 'P'))
                fixedEdges.push(getEdgesByKeys(0x68, "E0", 0x69, "W0"));
        }
    ...
    window.checkAutoAdjustments = function (makeAdjustments) {
        let info = {};
        let invalidScreensCount = 0, inconsistentGroupsCount = 0, missingScreensCount = 0, inconsistentSimilarCount = 0,
            inconsistentSwappedCount = 0, inconsistentParallelCount = 0, inconsistentCoupledCount = 0,
            invalidEdgesCount = 0, invalidIndegreeCount = 0, missingFixedCount = 0, missingSimilarCount = 0,
            missingSwappedCount = 0, missingParallelCount = 0, missingCoupledCount = 0;
        let inconsistentGroups = [], missingScreensNormal = [], missingScreensSwapped = [], invalidEdges = [],
            invalidIndegree = [], inconsistentEdges = [];
        if (mixedow) {
            if (worldState === 'S')
    ...
    window.explore = function (current, items, options, visitedRegions, visitedScreenEdges, checkableScreens, continueRegions, assumptions) {
        ...
        else if (current.openLogicEdgesOut)
    ...
    window.dijkstra = function (goal, items, options) {
        ...
        while (reachable.size != 0) {
            ...
            else if (current.openLogicEdgesIn)
    ...
    window.determineCheckableScreens = function (items, options, visitedRegions, visitedScreenEdges, checkableScreens, continueRegions) {
        ...
        let maybeVisitedRegions = new Set(), extraVisitedRegions = new Set(), maybeLightRegions = new Set(),
            maybeDarkRegions = new Set();
        if (mixedow && checkableScreens.size != 0) {//Find more checkable screens (bad algorithm, but works well enough to be useful)
            ...
            while (nextScreens.size != 0) {
                ...
                for (let id of cs) {
                    ...
                    let edgesAssumeNormal = new Set(visitedScreenEdges),
                        edgesAssumeSwapped = new Set(visitedScreenEdges);
                    let checkableAssumeNormal = new Set(checkableScreens),
                        checkableAssumeSwapped = new Set(checkableScreens);
                    ...
                    let diffNormal = setDifference(checkableAssumeNormal, new Set()),
                        diffSwapped = setDifference(checkableAssumeSwapped, new Set());
    ...
    window.determineLocationAvailability = function (items, r) {
            ...
            for (let location of region.locations) {
                if (data[location.type][location.id] !== "available" && region.screen.mixedState === "unknown") {
                    if (!location.rule || (checkRule(location.rule, items, options, true) && checkRule(location.rule, items, options, false))) {
                        data[location.type][location.id] = "available";
                    } else if (data[location.type][location.id] !== "darkavailable" && (checkRule(location.rule, items, options, true) || checkRule(location.rule, items, options, false)))
            ...
            for (let location of region.locations) {
                if (data[location.type][location.id] !== "available" && data[location.type][location.id] !== "darkavailable" && region.screen.mixedState === "unknown") {
                    if (!location.rule || (checkRule(location.rule, items, optionsDarkRooms, true) && checkRule(location.rule, items, optionsDarkRooms, false))) {
                        data[location.type][location.id] = "darkavailable";
                    } else if (data[location.type][location.id] !== "possible" && (checkRule(location.rule, items, optionsDarkRooms, true) || checkRule(location.rule, items, optionsDarkRooms, false)))
        ...
        if (mixedow) {
            if ((overworldScreens.get(0x1B).mixedState !== "unknown" && (overworldScreens.get(0x1B).mixedState === "flipped") === (worldState === 'I')) || (overworldScreens.get(0x03).mixedState !== "unknown" && (overworldScreens.get(0x03).mixedState === "flipped") === (worldState === 'I')))
                towerSwap = false;
            else if (overworldScreens.get(0x1B).mixedState !== "unknown" && (overworldScreens.get(0x1B).mixedState === "flipped") !== (worldState === 'I') && overworldScreens.get(0x03).mixedState !== "unknown" && (overworldScreens.get(0x03).mixedState === "flipped") !== (worldState === 'I'))
                towerSwap = true;
        ...
        if (!entranceEnabled) {
            ...
            else if (front !== "unavailable" && (items.bomb || items.boots) && (items.bomb || items.sword || items.bow > 1 || items.hammer || items.firerod || items.somaria || items.byrna)) {
            ...
            if (data.items[48] === "information" || data.items[48] === "unavailable")
                ...
                } else if (data.special["Desert Access"])
            ...
            if (items.mirror && items.hammer && items.moonpearl && data.items[4] === "unavailable" && data.entrances[136] !== "unavailable" && (!mixedow || overworldScreens.get(0x1B).mixedState !== "unknown") && (mixedow && overworldScreens.get(0x05).mixedState === "flipped") === (worldState === 'I'))
                if (door) {
                    ...
                    if (data.entrances[136] === "available" || data.entrances[136] === "darkavailable")
                        data.helpMimic = true;
                } else if (items.somaria && items.bomb)
                    if (retro || (items.flags && items.flags.wildkeys))
    ...
    window.andSpecial = function (object, locationID, special) {
        if (object[locationID] !== special)
            ...
            else if (((object[locationID] === "available" || object[locationID] === "darkavailable") && special === "information") || (object[locationID] === "information" && (special === "available" || special === "darkavailable")))
                object[locationID] = "information";
            else if (object[locationID] === "unavailable" || special === "unavailable" || !special || object[locationID] === "partialavailable" || special === "partialavailable" || object[locationID] === "information" || special === "information")
                object[locationID] = "unavailable";
            else if (object[locationID] === "darkpossible" || special === "darkpossible")
                object[locationID] = "darkpossible";
            else if ((object[locationID] === "possible" && special === "darkavailable") || (object[locationID] === "darkavailable" && special === "possible"))
                object[locationID] = "darkpossible";
            else if (object[locationID] === "possible" || special === "possible")
                object[locationID] = "possible";
            else if (object[locationID] === "darkavailable" || special === "darkavailable")
    ...
    window.getEntranceRegionFromIndex = function (index) {//This can cause problems in Mixed and Mystery. Change data structure in the future
    ...
    window.setVanillaTransition = function (screenID1, edgeID1, screenID2, edgeID2) {
        let edge1 = overworldScreens.get(screenID1).edges.get(edgeID1),
            edge2 = overworldScreens.get(screenID2).edges.get(edgeID2);
        edge1.vanilla = edge2;
        edge2.vanilla = edge1;
        if (edge1.parallel) {
