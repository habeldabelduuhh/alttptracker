(function (window) {
    ...
    window.doorCheck = function (dungeon, onlyDarkPossible, darkRoom, torchDarkRoom, posRequired, goal, onlyBunny = false) {
        if (flags.doorshuffle === 'N')
            return null;
        var doorcheck = 'available', bosscheck = onlyBunny ? 'unavailable' : door_enemizer_check(dungeon),
            wildsmallkeys = flags.wildkeys || flags.gametype === 'R';
        if (goal === 'boss')
            doorcheck = bosscheck;
    ...
    window.dungeonBoss = function (dungeonID, dungeonEntrances, dungeonEntrancesBunny) {
        ...
        switch (dungeonID) {
            ...
            case 1:
                if (flags.doorshuffle === 'N') {
                    var front = bestAvailability(dungeonEntrances[0], dungeonEntrances[1], dungeonEntrances[2]),
                        back = dungeonEntrances[3];
    ...
    window.dungeonChests = function (dungeonID, dungeonEntrances, dungeonEntrancesBunny) {
        ...
        switch (dungeonID) {
            ...
            case 11:
                if (flags.doorshuffle === 'N') {
                    var front = bestAvailability(dungeonEntrances[0], dungeonEntrances[1], dungeonEntrances[2]),
                        back = dungeonEntrances[4], sanc = dungeonEntrances[3];
