(function (window) {
    ...
    let outstandingUpdate = false, awaitingNextUpdate = true, awaitingResponse = false, enableAutoSave = false,
        tempSaveFile = null, tempCustomizerFile = null, initWidth = 0, initHeight = 0, currentDungeon = -1,
        currentPath = null, clickedPathIndex, currentlyEditing = false, editingIndex, globalHasBranch = false,
        roomModalClickAction = null, visibleSidebar = true, welcomeMode = true;
    let ownItems = {}, reachableEdges = null, showOverworldModeSection = false, showFullMapMain = true,
        expandedMainTable = true, showChecklist = false, currentOWPath = null, currentOWScreen = null,
        fullOverworldMode = null, fullOWFixedEdge = null, fullOWSelectedScreen = null, fullOWSelectedEdge = null,
        fullOWConnectorStart = null, fullOWConnectorEnd = null, extraOWScreen = null, extraOWDirection = null,
        fullOWPath = null, hoveredPathNumber = -1, lastUnknownConnectorIndex = -1, useMain = true, backToMain = true,
        editMapModeMain = true;
    let searchResults = new Map(), searchStartRegion = null, searchTargetRegion = null, searchStartScreen = null,
        searchAddCommonStarts = false, searchSaveQuitFluteEdges = false, searchResultsLength = 0,
        searchResultPaths = [], clickedOverworldPathIndex, clickedOverworldPathListName, clickedOverworldPath = null,
        clickedConnectorIndex, drawDarkWorldMain = false, drawDarkWorldPopout = false, mapModeMain = "single",
        mapModePopout = "single", mapModeAutoMain = true, mapModeAutoPopout = true, fullZoomMain = .8,
        fullZoomPopout = .8, fullZoomAutoMain = true, fullZoomAutoPopout = true, pathZoom = 1, fullPathCompact = true;
    let theme = "dark", invertDirectionColors = false, roundDirections = false, restoreWindowSize = true,
        defaultSync = "everything", autoSaveSafety = true, drawFluteSpots = "fluteshuffle",
        drawSaveQuitSpots = "startshuffle", hideTopBar = false, preventClickEdgeScreen = true, disableBlur = false,
        drawUnshuffledEdges = false, overworldMainColumns = "auto", searchStartDefault = "onlystartscreen",
        searchMirrorPortalDefault = false, importantRoomNodes = "default", omitForeignSymbols = true;
    window.dungeonPaths = [];
    ...
    const searchStartDefaultHelp = {
        "canstartcommon": "Takes both the selected screen and all common start points (save & quit spots) into account, mixed together.",
        "onlystartscreen": "Always starts at selected screen, but can save & quit to common starts and use flute (if available).",
        "noflutequit": "Always starts at selected screen and searches direct paths without using save & quit or flute."
    };
    const importantRoomNodesHelp = {
        "fewer": "Similar to \"Default\", but some of the simpler rooms are left out (mostly from groups of multiple similar rooms).",
        "default": "Roughly, these are the rooms or supertiles that are bounded by at least 5 screen transitions, or that play a special role (like TT Attic).",
        "more": "Roughly, these are the rooms or supertiles that are bounded by at least 4 screen transitions, or that play a special role (like TT Attic)."
    };
    const fullOWTitle = {
        "newpath": "Start a new path",
        "continuepath": "Where does this transition lead to?",
        "searchpath": "Select a target screen or preset",
        "searchpathtarget": "Select a new target screen",
        "searchpathstart": "Select a starting screen",
        "editedges": "Full overworld map",
        "editflutespots": "Click on screens to toggle flute spots"
    };
    ...
    window.receiveItemUpdate = function (newItems) {
        ...
        if (document.getElementById("itemsync").checked || document.getElementById("globalsync").checked) {
            ...
            } else if (newItems.glove === 1) {
                if (!ownItems.gloves || ownItems.mitts) {
                    ownItems.gloves = true;
                    ownItems.mitts = false;
                    changed = true;
                }
            } else if (!ownItems.gloves || !ownItems.mitts) {
    ...
    window.receiveMessage = function (event) {
        if (window.origin === event.origin) {
            ...
            } else if (!isNaN(event.data.tunic))
        ...
        try {
            if (newData.pinnedPaths)
                for (let pathData of newData.pinnedPaths)
                    ...
                    else if (Array.isArray(pinnedPaths.path))
        ...
        try {
            if (newData.previousPaths)
                for (let pathData of newData.previousPaths)
                    ...
                    else if (Array.isArray(previousPaths.path))
    ...
    window.connectorString = function (connector) {
        let regionStart = overworldScreens.get(connector.data[0]).regions.get(connector.data[1]),
            regionEnd = overworldScreens.get(connector.data[2]).regions.get(connector.data[3]);
    ...
    window.unknownConnectorOneWay = function (index) {
        if (entranceConnectors[index].status === "unknown") {
            ...
            updateConnectorList();
            drawFullOverworldPanels();
        } else if (entranceConnectors[index].status === "both") {
    ...
    window.clickedConnectorUnknown = function () {
        if (entranceConnectors[clickedConnectorIndex].status === "single") {
            ...
            updateConnectorList();
            drawFullOverworldPanels();
        } else if (entranceConnectors[clickedConnectorIndex].status === "both") {
    ...
    window.fullOverworldSwapScreen = function (button, includeEdges) {
        if (mixedow && fullOWSelectedScreen && fullOWSelectedScreen.mixedState !== "unknown") {
            if (includeEdges) {
                ...
                for (let id of group)
                    if (id < 0x40 || id >= 0x80) {
                        let screen = overworldScreens.get(id), gameLeft = screen.id % 0x40 === 0x28,
                            gameRight = screen.id % 0x40 === 0x29;
                        for (let edge of screen.edges.values())
    ...
    window.saveConnector = function () {
        let radioStart = document.querySelector("input[name='connectorstartgroup']:checked"),
            radioEnd = document.querySelector("input[name='connectorendgroup']:checked");
        if (radioStart && radioEnd) {
            let regionStart = fullOWConnectorStart.regions.get(radioStart.value),
                regionEnd = fullOWConnectorEnd.regions.get(radioEnd.value);
    ...
    window.saveCurrentOverworldPath = function () {
        for (let k = 3; k < currentOWPath.length; k += 3)
            if (!specialEdgeTypes.includes(currentOWPath[k - 2])) {
                let screen1 = overworldScreens.get(currentOWPath[k - 3]),
                    screen2 = overworldScreens.get(currentOWPath[k]);
    ...
    window.saveOverworldSearch = function () {
        ...
        else if (!searchSaveQuitFluteEdges)
    ...
    window.drawFullOverworldPanel = function (main) {
        ...
        for (let [id, screen] of overworldScreens) {
            ...
            let scale = .5, classes = "owscreen full",
                onClick = unknownScreen ? "" : "clickScreenFull(event," + id + "," + main + ")", valid = true;
            if (screen.big)
                classes += " bigscreen";
        ...
        if (mode === "editedges" && fullOWPath) {
            ...
            for (let k = 0; k < fullOWPath.path.length; k += 3) {
                if (k != 0 && !specialEdgeTypes.includes(fullOWPath.path[k - 1])) {
                    let screen1 = overworldScreens.get(fullOWPath.path[k - 3]),
                        screen2 = overworldScreens.get(fullOWPath.path[k]), scale = .5;
                    let edge1 = screen1.edges.get(fullOWPath.path[k - 2]),
                        edge2 = screen2.edges.get(fullOWPath.path[k - 1]);
    ...
    window.pickMapModeMain = function () {
        ...
        else if (Math.max(.5, Math.min(maxWidth / 512, (maxHeight + (hideTopBar ? 34 : 0)) / 1024)) >= Math.min(singleZoom * 7 / 8, .75))
    ...
    window.calculateFullZoomMain = function () {
        let maxWidth = document.body.clientWidth - (visibleSidebar ? 72 : 0),
            maxHeight = window.innerHeight - 252 + (hideTopBar && mapModeMain !== "single" ? 34 : 0);
    ...
    window.calculateFullZoomPopout = function () {
        let maxWidth = document.body.clientWidth - 14,
            maxHeight = window.innerHeight - 212 + (hideTopBar && mapModePopout !== "single" ? 34 : 0);
